...
import s from './tree-lines-pseudo.module.css';

export interface ArcManagerTreeLinesItem extends ArcManagerListItem {
  children?: ArcManagerTreeLinesItem[];
}

export interface ArcManagerTreeLinesProps {
  items: ArcManagerTreeLinesItem[];
  className?: string;
}

/**
 * ArcManagerTree - pseudo-element 기반 L자 라인 트리 변형
 *
 * - 각 행(row) 컨테이너에 CSS custom property(--tree-indent)를 심어서
 *   ::before / ::after가 들여쓰기 수준에 맞춰 L자 라인을 렌더링합니다.
 * - DOM 구조는 기본 ArcManagerTree와 거의 동일하게 유지합니다.
 */
export function ArcManagerTreeLines({ items, className }: ArcManagerTreeLinesProps) {
  ...
  const renderItem = (
    ...
    const itemWithIconClick: ArcManagerTreeLinesItem = {
      ...item,
